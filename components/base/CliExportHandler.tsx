      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            await new Promise<void>((resolve) => {
              ...
              const onLoaded = (event: Event) => {
                ...
                if (video.videoWidth > 0 && video.videoHeight > 0) {
                  console.log(
                    `Extracted video dimensions: ${video.videoWidth}x${video.videoHeight}`,
                  );
                  options.playResX = video.videoWidth;
                  options.playResY = video.videoHeight;
                } else {
                  console.log(
                    `Video metadata loaded but dimensions are invalid (Event: ${event.type})`,
                  );
                }
                resolve();
              };
          ...
          if (exportAssValue && files.length === 1) {
            ...
            outPath = await electronAPI.pathJoin(
              parsedPath.dir,
              (foundMediaName || parsedPath.name) + ".ass",
            );
          }
