...
const SyncCell = memo(
  ({
    ...
    if (!data) {
      return (
        <td
          className={`p-3 border-r border-[var(--app-border-base)] align-top bg-[var(--app-bg-input)]/30 text-center ${isDual ? "w-1/2" : "w-full"}`}
        >
          <span className="opacity-20 font-mono text-[10px] tracking-widest uppercase">
            Instrumental Break
          </span>
        </td>
      );
            ...
            <>
              <Tooltip title="編輯這行字RAW" delay={500}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditText(globalIndex);
                  }}
                  className={`p-1 px-1.5 text-[var(--app-text-muted)] hover:text-[var(--app-text-secondary)] hover:bg-[var(--app-bg-hover)] rounded transition-colors ${isDual ? "hidden 2xl:block" : "hidden xl:block"}`}
                >
              ...
              <Tooltip title={i18n.clearTimestamps} delay={500}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleClearTime(globalIndex);
                  }}
                  className={`p-1 px-1.5 text-[var(--app-text-muted)] hover:text-[var(--app-accent)] hover:bg-[var(--app-bg-hover)] rounded transition-colors ${isDual ? "hidden 2xl:block" : "hidden xl:block"}`}
                >
              ...
              <Tooltip title={i18n.deleteLine} delay={500}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleDeleteLine(globalIndex);
                  }}
                  className="p-1 px-1.5 text-[var(--app-text-muted)] hover:text-red-500 hover:bg-[var(--app-bg-hover)] rounded transition-colors"
                >
              ...
              <Tooltip title="其他選項" delay={500}>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    openMoreMenu(e, globalIndex);
                  }}
                  className="p-1 px-1.5 text-[var(--app-text-muted)] hover:text-[var(--app-text-secondary)] hover:bg-[var(--app-bg-hover)] rounded transition-colors"
                >
  ...
  (prevProps, nextProps) => {
    ...
    return (
      prevProps.data.line === nextProps.data.line &&
...
export function SyncEditor() {
      ...
      if (paragraphStarts[i]) {
        if (i === 0) {
          ...
          if (prevEnd === null && prevLine.words?.length > 0) {
            const lastWordWithStart = [...prevLine.words]
              .reverse()
              .find((w: any) => w.start !== null);
  ...
  const [ktvspDialog, setKtvspDialog] = useState<{ globalIndex: number; text: string } | null>(
    null,
  );
  ...
  const handleEditTextOnly = useCallback(
    (globalIndex: number) => {
  ...
  const handleOffsetFromHere = useCallback(
    ...
    (globalIndex: number) => {
      commitLines((prev) => {
        ...
        if (newLines[globalIndex].words) {
          newLines[globalIndex].words = newLines[globalIndex].words.map((w: any) => ({
            ...w,
            start: null,
            end: null,
          }));
  ...
  const handleSetLineStyle = useCallback(
    (globalIndex: number, styleId: string) => {
      commitLines((prev) => {
        const newLines = [...prev];
  ...
  const handleSetWordStyle = useCallback(
    (globalIndex: number, wordIndex: number, styleId: string) => {
      commitLines((prev) => {
        const newLines = [...prev];
        const newWords = [...newLines[globalIndex].words];
        newWords[wordIndex] = {
          ...newWords[wordIndex],
          style: newWords[wordIndex].style === styleId ? undefined : styleId,
        };
  ...
  const handleMergeToPrevious = useCallback(
    (globalIndex: number) => {
  ...
  const handleEditTimeOnly = useCallback(
    async (globalIndex: number) => {
      const line = lines[globalIndex];
      const val = await dialogs.prompt(
        "Edit timestamp (ss.xx or mm:ss.xx):",
        line.start !== null ? formatTime(line.start, true) : "",
      );
      if (val !== null) {
  ...
  const handleJumpTo = useCallback(
    (globalIndex: number, wordIndex?: number) => {
      const line = lines[globalIndex];
      if (!line) return;
      const { current: player } = playerRef;
  ...
  const handleSplitWordToNextLine = useCallback(
    (globalIndex: number, wordIndex: number) => {
      commitLines((prev) => {
        const newLines = [...prev];
        const line = newLines[globalIndex];
        const words = line.words;
        const lineHasEndStamp =
          words.length > 0 &&
          words[words.length - 1].text === "" &&
          words[words.length - 1].start !== null;
  ...
  const handleToggleSingleLine = useCallback(
    (globalIndex: number) => {
      commitLines((prev) => {
  ...
  const handleEditWordRaw = useCallback(
    async (globalIndex: number, wordIndex: number) => {
      const line = lines[globalIndex];
      const word = line.words[wordIndex];
      const currentText =
        word.start !== null ? `<${formatTime(word.start, true)}>${word.text}` : word.text;
      ...
      if (val !== null && val !== currentText) {
        commitLines((prev) => {
          ...
          const resultWords = parsedWordLine
            ? parsedWordLine.words
            : [{ text: val, start: null, end: null }];
  ...
  const handleEditWordText = useCallback(
    async (globalIndex: number, wordIndex: number) => {
      const line = lines[globalIndex];
      const word = line.words[wordIndex];
  ...
  const uiRows: {
    left: { line: any; index: number } | null;
    right?: { line: any; index: number } | null;
  }[] = [];
  ...
  const handleLineConvertToTraditional = async (globalIndex: number) => {
    ...
    commitLines((prev) => {
      ...
      newLines[globalIndex] = {
        ...newLines[globalIndex],
        words: newLines[globalIndex].words.map((w) => ({
          ...w,
          text: convertToTraditional(w.text),
        })),
      };
      newLines[globalIndex].raw = newLines[globalIndex].words
        .map((w) => (w.start !== null ? `<${formatTime(w.start, true)}>${w.text}` : w.text))
        .join("");
      return newLines;
    }, "這行字轉繁體");
  ...
  const handleLineConvertToSimplified = async (globalIndex: number) => {
    ...
    commitLines((prev) => {
      ...
      newLines[globalIndex] = {
        ...
        words: newLines[globalIndex].words.map((w) => ({
          ...w,
          text: convertToSimplified(w.text),
        })),
      };
      newLines[globalIndex].raw = newLines[globalIndex].words
        .map((w) => (w.start !== null ? `<${formatTime(w.start, true)}>${w.text}` : w.text))
  ...
  const handleWordConvertToTraditional = async (globalIndex: number, wordIndex: number) => {
    ...
    commitLines((prev) => {
      ...
      newWords[wordIndex] = {
        ...newWords[wordIndex],
        text: convertToTraditional(newWords[wordIndex].text),
      };
      newLines[globalIndex] = { ...newLines[globalIndex], words: newWords };
      newLines[globalIndex].raw = newLines[globalIndex].words
        .map((w) => (w.start !== null ? `<${formatTime(w.start, true)}>${w.text}` : w.text))
  ...
  const handleWordConvertToSimplified = async (globalIndex: number, wordIndex: number) => {
    ...
    commitLines((prev) => {
      ...
      newWords[wordIndex] = {
        ...newWords[wordIndex],
        text: convertToSimplified(newWords[wordIndex].text),
      };
      newLines[globalIndex] = { ...newLines[globalIndex], words: newWords };
      newLines[globalIndex].raw = newLines[globalIndex].words
        .map((w) => (w.start !== null ? `<${formatTime(w.start, true)}>${w.text}` : w.text))
    ...
    <div className="contents lg:flex lg:flex-col lg:h-full lg:bg-[var(--app-bg-base)]">
      <div className="p-3 bg-[var(--app-bg-panel-alt)] flex flex-wrap items-center justify-between shrink-0 gap-2 lg:static z-20">
        ...
        <div className="flex flex-wrap items-center gap-4 text-[10px] text-[var(--app-text-muted)]">
          ...
          <button
            ...
            onClick={() => (syncMode === "line" ? handleLineStamp() : handleWordStamp())}
            className="bg-[var(--app-bg-panel)] hover:bg-[var(--app-border-base)] transition-colors p-1.5 rounded border border-[var(--app-border-base)] flex items-center gap-2 shadow-sm cursor-pointer select-none text-xs h-[30px]"
          >
            <span className="uppercase">{i18n.timestampWords}</span>
            <kbd className="bg-[var(--app-bg-base)] text-[var(--app-accent)] px-1.5 py-0.5 rounded font-mono border border-[var(--app-border-light)]">
              {hotkeys.stampWord === " " ? "SPACE" : hotkeys.stampWord}
            </kbd>
          </button>
          ...
          {syncMode === "word" && (
            <button
              onFocus={(e) => e.target.blur()}
              onClick={() => handleWordNextLine()}
              className="bg-[var(--app-bg-panel)] hover:bg-[var(--app-border-base)] transition-colors p-1.5 rounded border border-[var(--app-border-base)] flex items-center gap-2 shadow-sm cursor-pointer select-none text-xs h-[30px]"
            >
              <span className="uppercase">{i18n.nextLine}</span>
              <kbd className="bg-[var(--app-bg-base)] text-[var(--app-accent)] px-1.5 py-0.5 rounded font-mono border border-[var(--app-border-light)]">
                {hotkeys.nextLine.toUpperCase()}
              </kbd>
            </button>
          )}
        </div>
      ...
      >
        <table className="w-full text-left text-xs border-collapse table-fixed">
          <thead
            className={`${!isMobile || isTall ? "sticky top-0" : ""} bg-[var(--app-bg-panel)] text-[var(--app-text-muted)] z-10 text-[10px] uppercase tracking-widest font-bold outline outline-1 outline-b-[var(--app-border-base)]`}
          >
            <tr>
              {isDual ? (
                <>
                  <th className="p-3 w-1/2 border-r border-[var(--app-border-base)]">
                    {i18n.leftTrack}
                  </th>
                  <th className="p-3 w-1/2">{i18n.rightTrack}</th>
                </>
              ) : (
                <th className="p-3 w-full border-r border-[var(--app-border-base)]">
                  {i18n.lyricsContentEnhanced}
                </th>
              )}
            </tr>
          </thead>
          ...
          {ctxMenu.type === "line" && (
            <>
              <ContextMenuItem
                ...
                label="歌曲跳轉至該行開頭"
                onClick={() => {
                  handleJumpTo(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="複製這行字"
                onClick={() => {
                  navigator.clipboard.writeText(getLineText(lines[ctxMenu.globalIndex]));
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                onClick={() => {
                  navigator.clipboard.writeText(
                    lines[ctxMenu.globalIndex]?.start !== null
                      ? formatTime(lines[ctxMenu.globalIndex].start!)
                      : "",
                  );
                  setCtxMenu(null);
                }}
              ...
              <ContextMenuItem
                ...
                label="複製這行字RAW（含時間戳）"
                onClick={() => {
                  navigator.clipboard.writeText(getLineRawText(lines[ctxMenu.globalIndex]));
                  setCtxMenu(null);
                }}
              />
              <ContextMenuSeparator />
              <ContextMenuItem
                ...
                label="編輯這行字"
                onClick={() => {
                  handleEditTextOnly(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="編輯這行字RAW（含時間戳）"
                onClick={() => {
                  handleEditRawText(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuSeparator />
              <ContextMenuSub
                icon={
                  <span className="w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] bg-gradient-to-br from-blue-500 to-red-500 text-white">
                    🎨
                  </span>
                }
                label="顏色標記"
              >
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.style === "B" ? "text-white bg-blue-500" : "text-blue-500"}`}
                    >
                      B
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.style === "B"
                      ? "✓ 取消標記藍色"
                      : "設為藍色B (男 or 第一人)"
                  }
                  onClick={() => {
                    handleSetLineStyle(ctxMenu.globalIndex, "B");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.style === "R" ? "text-white bg-red-500" : "text-red-500"}`}
                    >
                      R
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.style === "R"
                      ? "✓ 取消標記紅色"
                      : "設為紅色R (女 or 第二人)"
                  }
                  onClick={() => {
                    handleSetLineStyle(ctxMenu.globalIndex, "R");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.style === "P" ? "text-white bg-purple-500" : "text-purple-500"}`}
                    >
                      P
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.style === "P"
                      ? "✓ 取消標記紫色"
                      : "設為紫色P (第三人)"
                  }
                  onClick={() => {
                    handleSetLineStyle(ctxMenu.globalIndex, "P");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.style === "G" ? "text-white bg-green-500" : "text-green-500"}`}
                    >
                      G
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.style === "G"
                      ? "✓ 取消標記綠色"
                      : "設為綠色G (合唱)"
                  }
                  onClick={() => {
                    handleSetLineStyle(ctxMenu.globalIndex, "G");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.style === "T" ? "text-white bg-gray-500" : "text-gray-500"}`}
                    >
                      T
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.style === "T"
                      ? "✓ 取消標記灰色"
                      : "設為灰色T (旁白 or 隱藏)"
                  }
                  onClick={() => {
                    handleSetLineStyle(ctxMenu.globalIndex, "T");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                ...
                <ContextMenuItem
                  ...
                  label={
                    lines[ctxMenu.globalIndex]?.style === "N" ? "✓ 取消回歸預設" : "設為回歸預設N"
                  }
                  onClick={() => {
                    handleSetLineStyle(ctxMenu.globalIndex, "N");
                    setCtxMenu(null);
                  }}
                />
              </ContextMenuSub>
              ...
              <ContextMenuItem
                ...
                label={
                  lines[ctxMenu.globalIndex]?.isSingleLine
                    ? "✓ 算做第三句 (已啟用)"
                    : "算做第三句（強制單行）"
                }
                onClick={() => {
              ...
              <ContextMenuItem
                icon={
                  <span className="w-3.5 h-3.5 flex items-center justify-center font-bold text-[10px] text-green-500">
                    SP
                  </span>
                }
                label={
                  lines[ctxMenu.globalIndex]?.ktvsp != null
                    ? `✓ 特殊作為段落開始 (${formatTime(lines[ctxMenu.globalIndex].ktvsp!)})`
                    : "特殊作為段落開始 (ktvsp)"
                }
                onClick={() => {
                  ...
                  setKtvspDialog({
                    globalIndex: ctxMenu.globalIndex,
                    text:
                  ...
                  setTimeout(
                    () =>
                      window.dispatchEvent(
                        new CustomEvent("focus-raw-text-line", {
                          detail: { lineIndex: targetIndex },
                        }),
                      ),
                    50,
                  );
              ...
              <ContextMenuItem
                ...
                label="插入上一行"
                onClick={() => {
                  handleInsertLineBefore(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="插入下一行"
                onClick={() => {
                  handleInsertLineAfter(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuSeparator />
              <ContextMenuItem
                ...
                label="合併到上一行"
                onClick={() => {
                  handleMergeToPrevious(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="平移後續時間"
                onClick={() => {
                  handleOffsetFromHere(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="清除時間戳"
                onClick={() => {
                  handleClearTime(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="刪除行"
                onClick={() => {
                  handleDeleteLine(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <div className="px-3 py-2 flex flex-col gap-0.5 border-t border-[var(--app-border-base)]/50 mb-1 pointer-events-none">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-[10px] text-[var(--app-text-muted)] font-bold uppercase tracking-widest">
                    目前選擇的行
                  </span>
                ...
                <span className="text-sm font-bold text-[var(--app-text-primary)] truncate max-w-[200px]">
                  {getLineText(lines[ctxMenu.globalIndex]) || "(空白行)"}
                </span>
              </div>
            </>
          )}
          ...
          {ctxMenu.type === "word" && ctxMenu.wordIndex !== undefined && (
            <>
              <ContextMenuItem
                ...
                label="歌曲跳轉至該字"
                onClick={() => {
                  handleJumpTo(ctxMenu.globalIndex, ctxMenu.wordIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                onClick={() => {
                  navigator.clipboard.writeText(
                    lines[ctxMenu.globalIndex].words[ctxMenu.wordIndex!].text,
                  );
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="複製字的時間戳"
                onClick={() => {
                  const word = lines[ctxMenu.globalIndex].words[ctxMenu.wordIndex!];
                  navigator.clipboard.writeText(
                    word.start !== null ? formatTime(word.start, true) : "",
                  );
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                onClick={() => {
                  ...
                  navigator.clipboard.writeText(
                    word.start !== null
                      ? `<${formatTime(word.start, true)}>${word.text}`
                      : word.text,
                  );
                  setCtxMenu(null);
                }}
              ...
              <ContextMenuItem
                ...
                label="編輯這段字"
                onClick={() => {
                  handleEditWordText(ctxMenu.globalIndex, ctxMenu.wordIndex!);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="編輯這段字RAW（含時間戳）"
                onClick={() => {
                  handleEditWordRaw(ctxMenu.globalIndex, ctxMenu.wordIndex!);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuSeparator />
              ...
              >
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "B" ? "text-white bg-blue-500" : "text-blue-500"}`}
                    >
                      B
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "B"
                      ? "✓ 取消標記藍色"
                      : "設為藍色B (男 or 第一人)"
                  }
                  onClick={() => {
                    handleSetWordStyle(ctxMenu.globalIndex, ctxMenu.wordIndex!, "B");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "R" ? "text-white bg-red-500" : "text-red-500"}`}
                    >
                      R
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "R"
                      ? "✓ 取消標記紅色"
                      : "設為紅色R (女 or 第二人)"
                  }
                  onClick={() => {
                    handleSetWordStyle(ctxMenu.globalIndex, ctxMenu.wordIndex!, "R");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "P" ? "text-white bg-purple-500" : "text-purple-500"}`}
                    >
                      P
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "P"
                      ? "✓ 取消標記紫色"
                      : "設為紫色P (第三人)"
                  }
                  onClick={() => {
                    handleSetWordStyle(ctxMenu.globalIndex, ctxMenu.wordIndex!, "P");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "G" ? "text-white bg-green-500" : "text-green-500"}`}
                    >
                      G
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "G"
                      ? "✓ 取消標記綠色"
                      : "設為綠色G (合唱)"
                  }
                  onClick={() => {
                    handleSetWordStyle(ctxMenu.globalIndex, ctxMenu.wordIndex!, "G");
                    setCtxMenu(null);
                  }}
                />
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "T" ? "text-white bg-gray-500" : "text-gray-500"}`}
                    >
                      T
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "T"
                      ? "✓ 取消標記灰色"
                      : "設為灰色T (旁白 or 隱藏)"
                  }
                  onClick={() => {
                ...
                <ContextMenuItem
                  icon={
                    <span
                      className={`w-3.5 h-3.5 rounded flex items-center justify-center font-bold text-[10px] ${lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "N" ? "text-white bg-[var(--app-accent)]" : "text-[var(--app-accent)]"}`}
                    >
                      N
                    </span>
                  }
                  label={
                    lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.style === "N"
                      ? "✓ 取消回歸預設"
                      : "設為回歸預設N"
                  }
                  onClick={() => {
                    handleSetWordStyle(ctxMenu.globalIndex, ctxMenu.wordIndex!, "N");
                    setCtxMenu(null);
                  }}
                />
              </ContextMenuSub>
              ...
              <ContextMenuItem
                ...
                label="這段字轉繁體"
                onClick={() => {
                  handleWordConvertToTraditional(ctxMenu.globalIndex, ctxMenu.wordIndex!);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
                ...
                label="這段字轉簡體"
                onClick={() => {
                  handleWordConvertToSimplified(ctxMenu.globalIndex, ctxMenu.wordIndex!);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuSeparator />
              <ContextMenuItem
                ...
                label="從該字分割斷行到下一行"
                onClick={() => {
                  handleSplitWordToNextLine(ctxMenu.globalIndex, ctxMenu.wordIndex!);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
              ...
              <ContextMenuItem
                ...
                label="刪除該段字"
                onClick={() => {
                  handleDeleteWord(ctxMenu.globalIndex, ctxMenu.wordIndex!);
                  setCtxMenu(null);
                }}
              />
              <div className="px-3 py-2 flex flex-col gap-0.5 border-t border-[var(--app-border-base)]/50 mb-1 pointer-events-none">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-[11px] font-mono text-[var(--app-text-muted)] opacity-60">
                    {lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.start !== null
                      ? formatTime(
                          lines[ctxMenu.globalIndex].words[ctxMenu.wordIndex!].start!,
                          true,
                        )
                      : "--:--.--"}
                  </span>
                ...
                <span className="text-sm font-bold text-[var(--app-text-primary)] truncate max-w-[200px]">
                  {lines[ctxMenu.globalIndex]?.words[ctxMenu.wordIndex!]?.text || "(無)"}
                </span>
              </div>
            </>
          )}
          ...
          {ctxMenu.type === "time" && (
            <>
              ...
              <ContextMenuItem
                ...
                label="編輯這行時間戳（手打輸入）"
                onClick={() => {
                  handleEditTimeOnly(ctxMenu.globalIndex);
                  setCtxMenu(null);
                }}
              />
              <ContextMenuItem
              ...
              onClick={() => {
                ...
                commitLines((prev) => {
                  ...
                  if (editingText.type === "raw") {
                    ...
                    newLines[editingText.globalIndex] = {
                      ...currentLine,
                      raw: editingText.text,
                      words: parsed ? parsed.words : [],
                    };
                  } else {
                    newLines[editingText.globalIndex] = {
                      ...currentLine,
                      raw: editingText.text,
                      words: splitWordsAegisub(editingText.text),
                      start: null,
                    };
                  }
                  return newLines;
      ...
      >
        {editingText && (
          <div className="flex flex-col gap-3">
            ...
            />
            <div className="text-xs text-[var(--app-text-muted)] font-bold uppercase tracking-widest mt-2 px-1">
              拆字預覽
            </div>
            <div className="flex flex-wrap gap-1 p-3 bg-[var(--app-bg-input)] rounded-lg border border-[var(--app-border-light)] min-h-[3rem] items-center">
              {(() => {
                const { parseRawLyrics, splitWordsAegisub } = require("@/lib/lyric-utils");
                const words =
                  editingText.type === "raw"
                    ? parseRawLyrics(editingText.text).lines[0]?.words || []
                    : splitWordsAegisub(editingText.text);

                if (words.length === 0)
                  return (
                    <span className="opacity-50 text-xs text-[var(--app-text-muted)]">無內容</span>
                  );
                ...
                return words.map((w: any, i: number) => (
                  ...
                  >
                    {w.text || "⏎"}
                    {w.start !== null && (
                      <span className="absolute -top-2 -right-2 text-[8px] bg-[var(--app-accent)] text-black px-1 rounded font-bold">
                        {formatTime(w.start, true)}
                      </span>
                    )}
                  </span>
                ));
          ...
          <>
            <button
              onClick={() => {
                ...
                commitLines((prev) => {
                  const newLines = [...prev];
                  newLines[ktvspDialog.globalIndex] = {
                    ...newLines[ktvspDialog.globalIndex],
                    ktvsp: undefined,
                  };
                  return newLines;
      ...
      <MultiSingerDialog
        isOpen={multiSingerDialogOpen}
        onClose={() => setMultiSingerDialogOpen(false)}
      />
    </div>
  );
}
