...
import { LrcMetadata, LyricLine } from '@/lib/lyric-utils';
import { Settings2, Split, Merge } from 'lucide-react';
...
export function MultiSingerDialog({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const handleSaveDef = () => {
    ...
    if (bDef) nextMeta.kstyledef_B = bDef; else delete nextMeta.kstyledef_B;
    if (rDef) nextMeta.kstyledef_R = rDef; else delete nextMeta.kstyledef_R;
    if (pDef) nextMeta.kstyledef_P = pDef; else delete nextMeta.kstyledef_P;
    if (gDef) nextMeta.kstyledef_G = gDef; else delete nextMeta.kstyledef_G;
    if (tDef) nextMeta.kstyledef_T = tDef; else delete nextMeta.kstyledef_T;
            ...
            if (firstWordText.startsWith(pattern)) {
              ...
              for (let w = 1; w < words.length; w++) {
                if (words[w].text.startsWith(pattern)) {
                  words[w] = { ...words[w], text: words[w].text.substring(pattern.length), style: def.key };
                  changedLine = true;
                }
              }
            }
